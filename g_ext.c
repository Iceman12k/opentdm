...
edict_t *xerp_ent;
trace_t XERP_trace(vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end)
{
...
int G_customizeentityforclient(edict_t *client, edict_t *ent, entity_state_t *state)
{
	...
	if (!(macromax(1, ent->dimension_visible) & macromax(1, client->client->dimension_observe)))
		return false;
	...
	if (ent->client) // extrapolate clients
	{
		...
		vec3_t vel;
		vel[0] = pm.s.velocity[0] / 8;
		vel[1] = pm.s.velocity[1] / 8;
		vel[2] = pm.s.velocity[2] / 8;
		if ((VectorLength(vel)) < 280)
		{
			pm.cmd.sidemove = 0;
